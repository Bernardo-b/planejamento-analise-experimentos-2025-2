...
## 2.1 Cálculo Amostral (Análise de Poder)
...
## 3.2 Verificação de Pressupostos (Análise Pareada)
...
## 3.3 Análise Estatística (Teste de Wilcoxon Pareado)
...
```{r wilcoxon_test, echo=FALSE}
# Este é o teste principal. 
# Note o 'paired = TRUE' e 'conf.int = TRUE'
wilcox_res <- wilcox.test(dados_long$Fbest ~ dados_long$Configuracao, 
                          data = dados_long,
                          paired = TRUE, 
                          alternative = "two.sided", 
                          conf.int = TRUE)
...
Embora a análise metodológica tenha sido rigorosa, é importante reconhecer as fronteiras das conclusões deste estudo. Identificamos três limitações principais que não invalidam os resultados, mas definem seu escopo:

1.  **Foco na Qualidade Final vs. Velocidade de Convergência:**
    O experimento foi desenhado para medir a *qualidade final* da solução (`Fbest`) após um orçamento fixo de avaliações (30 execuções de `5000 * D` avaliações cada). O estudo prova conclusivamente que a Cfg1 é 3x melhor no final. No entanto, o estudo não mediu a **velocidade de convergência**. É possível que a Cfg2 (mesmo sendo pior no final) encontre uma solução "boa o suficiente" muito mais rápido. Para aplicações onde o tempo de resposta é o fator crítico, uma análise do desempenho ao longo do tempo seria necessária.

2.  **Métrica de Desempenho (Média vs. Variabilidade):**
...
3.  **Custo Computacional (Avaliações vs. Tempo de Relógio):**
    A comparação foi baseada em um orçamento idêntico de "avaliações de função". No entanto, as operações das duas configurações têm custos computacionais muito diferentes. A Cfg1 usa `mutation_best`, que exige que o algoritmo, a cada passo, encontre o melhor indivíduo da população — uma operação mais custosa (em tempo de relógio) do que a `mutation_rand` da Cfg2, que apenas sorteia vetores. Portanto, o estudo prova que a Cfg1 é mais *eficiente* (dá um resultado melhor para o mesmo número de avaliações), mas ela é quase certamente mais *lenta* em segundos (tempo real).
