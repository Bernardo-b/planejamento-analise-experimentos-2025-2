# Análise Estatística da Acurácia - Experimento RCBD
...
cat("Ajustando modelo RCBD: acuracia ~ metodo + bloco\n\n")
modelo <- aov(acuracia ~ metodo + bloco, data = df)
...
png("results/qqplot_acuracia.png", width = 800, height = 600)
plot(modelo, 2)  # Plot 2 do modelo aov é o QQ-Plot
dev.off()
cat("Arquivo salvo: results/qqplot_acuracia.png\n\n")

# ============================================================================
# 4. TESTES DE PREMISSAS E ANÁLISE ESTATÍSTICA
# ============================================================================

sink("results/relatorio_estatistico_acuracia.txt")

cat("================================================================================\n")
cat("RELATÓRIO DE ANÁLISE ESTATÍSTICA - ACURÁCIA (RCBD)\n")
cat("================================================================================\n\n")
...
cat("--------------------------------------------------------------------------------\n")
teste_fligner <- fligner.test(acuracia ~ metodo, data = df)
print(teste_fligner)
cat("\n")
...
if (teste_shapiro$p.value > 0.05) {
  ...
  cat("--------------------------------------------------------------------------------\n")
  teste_friedman <- friedman.test(acuracia ~ metodo | bloco, data = df)
  ...
  if (teste_friedman$p.value < 0.05) {
    ...
    cat("--------------------------------------------------------------------------------\n")
    teste_wilcox <- pairwise.wilcox.test(df$acuracia, df$metodo,
...
resumo <- data.frame(
  ...
  Média = sapply(metodos, function(m) mean(df$acuracia[df$metodo == m])),
  Mediana = sapply(metodos, function(m) median(df$acuracia[df$metodo == m])),
  SD = sapply(metodos, function(m) sd(df$acuracia[df$metodo == m])),
  Q1 = sapply(metodos, function(m) quantile(df$acuracia[df$metodo == m], 0.25)),
  Q3 = sapply(metodos, function(m) quantile(df$acuracia[df$metodo == m], 0.75)),
  Min = sapply(metodos, function(m) min(df$acuracia[df$metodo == m])),
  Max = sapply(metodos, function(m) max(df$acuracia[df$metodo == m]))
)
...
cat("\n\nTABELA COMPLETA - Todas as Estatísticas:\n\n")
...
cat("- IQR: Intervalo Interquartil (mede consistência - menor é melhor)\n")
cat("- Diff_Absoluta: Diferença da mediana em relação ao GridSearch\n")
cat("- Melhoria_Pct: Percentual de melhoria/piora em relação ao GridSearch\n")
cat("  Positivo = melhor que GridSearch | Negativo = pior que GridSearch\n")
...
cat("✓ Gráficos salvos em: results/qqplot_acuracia.png\n")
cat("✓ Relatório salvo em: results/relatorio_estatistico_acuracia.txt\n")
cat("\n")
